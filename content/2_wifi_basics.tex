...
\begin{figure}[h]
    \centering 
    \includegraphics[width=0.6\linewidth]{figures/MAC_Frame.png}
    \label{fig:mac_frame}
\end{figure}
...
\begin{figure}[h]
    \centering 
    \includegraphics[width=0.6\linewidth]{figures/Connection_Establishment.png}
...
Cybersecurity aims to protect devices and data by achieving the CIA triad, which is an acronym for Confidentiality, Integrity, and Availability \cite[chapter~Attacking Integrity Controls]{Oriyano_2017} .
...
Encryption is a cryptographic \footnote{cryptography = science of secretive writing} method, which takes the readable data, called plaintext, and makes it unreadable for anyone besides the recipient.
...
WPA still uses RC4 for compatibility reasons, but the encryption is based on a pairwise master key, which is generated for each STA after it has authenticated successfully.
As well as using a longer 128-Bit preshared key (PSK), the TKIP encryption dynamically changes the encryption key for each packet.
...
AES-CCMP encrypts data blockwise and uses a compbination of substitution and transposition algorithms to acheive high security.
...
Authentication is the act of a user proving, that he is authorised to access something.
This is oftentimes done by asking for a secret, in the case of WiFi networks, the knowledge of the password proves, that a user is allowed to access the network.
...
WPA3 replaces the PSK by simultaneous authentication of equals (SAE)
