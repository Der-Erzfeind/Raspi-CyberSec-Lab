...
Switching the configuration back to WPA-PSK, the Wi-Fi connection continued trying to reach the RADIUS server for enterprise authentication during activation, timing out with the error message:
...
These signals are called DT and CLK, which, when turned, oscillate with 90 degrees of phase shift, resulting in a Gray code with two bits.
...
The discovery of this behavior helped to simplify the processing even easier since this pattern allows a single query of the signal states to determine the direction of rotation.
