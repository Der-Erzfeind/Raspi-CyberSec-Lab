...
The Fenvi AX1800 USB Wi-Fi card was chosen for its compatibility with the Linux kernel, according to \cite{morrownr_USB_Wi-Fi}, and low cost.
...
\subsection{Overview}
The software of the project is mostly modular, and it consists of a central program, which runs the user interface (UI) and then executes various shell\footnote{Terminal of a Linux/Unix system for entering commands} scripts, with which the functionality is implemented.
As can be seen in \cref{fig:architecture}, besides the UI program, there are programs for MQTT communication and input processing, also the services of the Network Manager and Juice Shop project are used. 
...
An overview of the menus and options, currently present inside the UI program can be seen in \cref{tab:menus_options}.
...
During navigation, the UI periodically prints the available options, using Unicode to highlight the currently selected option as well as to give contrast to the title of the menu.
