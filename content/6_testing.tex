...
WEP encryption is based on the RC4 algorithm and works "by exclusive-ORing the data stream with a pseudorandom stream of bits generated based on the WEP key" \cite[page~104]{Sankaran_Gulasekaran_2021} and a 24-bit initialization vector (IV).
...
The probability of the same key beeing reused is at about 50\% after around 5000 frames have been captured, opening the possibility for reverse engineering by analysis of the frames \cite{Oriyano_2017}.
...
In normal operation a NIC only recieves the traffic, adressed to its MAC address.
To disable this filtering, the network card is put into monitor mode, allowing it to capture all packets transmitted in the vacinity, and discover all APs present.
...
This technique is called a replay attack and involves the capture of serveral authentic ARP frames, subsequently modifying and injecting them, which "the AP will re-broadcast [...] and as a result generate [new IVs]" \cite{Oriyano_2017}.

Sending ARP frames without association to the AP will lead to them beeing rejected, therefore another spoofed frame is injected, to fake authentication and association with the AP.
...
Thus a Deauthentication Attack is performed, injecting deauthentication frames, which force the STAs reconnect to the AP and therefore making the capture of a handshake predictable.
...
The Aircrack-ng suite is taylored towards network security assessment and is included with Kali Linux.
...
With the card beeing able to receive all traffic, the target network can be searched, by using \lstinline[]|airodump-ng wlan1mon|.
...
As seen in \cref{fig:airodump-ng}, the program scans on all channel for networks within reach and lists them in the table on top.
...
Upon successful association, to be seen in \cref{fig:aireplay-ng}, ARP requests can be injected.
...
Creating and capturing traffic, IVs are collected and the cracking of the password can be started simlutaniously. 
...
The output, depicted in \cref{fig:aireplay_test_WPA}, means that normal frames are ignored, due to the STA not beeing authenticated.
Because PMF is not active, deauthentication frames can still be injected.
...
\\\lstinline[]|-a| specifies the APs BSSID, leading to the frames beeing broadcasted to all STAs
...
Aircrack-ng starts to interate through the password list, computes the PSK for each password and compares it to the captured PSK.
...
Navigation of the UI seems intuitive and switching between menues works fluidly.
Execution of the various scripts is reliable, if implemented correctly.
However the ESP32 has been observed to crash occasionally during WiFi connection establishment.
The circumstances and cause for the occurence of these sporadic crashes are unclear at the moment and a fix is not forseen for the scope of this thesis.
...
The webapp functions, specifially the MQTT communication and Juice Shop server, seem to work without complication, but were not tested thoroughly.
...
This time the password could be cracked after around 3 minutes and only 214 captured IVs.

For control, a third test was conducted, for which the password of the hotspot was changed with the new password function.
Again the password could be cracked in a few minutes, with around 15000 IVs collected.
...
Cause for the failing deauthentication attacks most likely is PMF beeing used for the network.
Even though the configuration was double checked, the previous unexpected behavior of Network Manager leads to the suspicion that the configuration is not beeing applied correctly.
