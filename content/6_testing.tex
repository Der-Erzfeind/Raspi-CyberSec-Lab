...
With the card being able to receive all traffic, the target network can be searched by using \lstinline[]|airodump-ng wlan1mon|.
...
\subsection{Cracking WEP}
The first attempt at cracking the WEP network was unsuccessful because only the ESP32 being connected to the network, ARP request injection cannot generate significant traffic, therefore the collection of IVs proceeds very slowly.
...
Cause for the failing deauthentication attacks is most likely PMF being used for the network.
